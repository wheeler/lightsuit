...
static int flicker_low_min = .5 * led_on;
static int flicker_low_max = .85 * led_on;
 
// the end value of the flicker (high)
static int flicker_high_min = .8 * led_on;
static int flicker_high_max = led_on;
...
void flicker()
{
  ...
  // low -> high
  for (int i = flicker_random_low_start; i<flicker_random_high; i+=16)
  {
  ...
  // high -> low
  for (int i = flicker_random_high; i>=flicker_random_low_end; i-=16)
  {
    for (pin=0; pin<=num_levels; pin++)
      set_suit_level(pin, 0, i); 
